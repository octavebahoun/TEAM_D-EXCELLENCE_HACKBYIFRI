...
export default function ResponsableCommunications() {
  ...
  const fetchCommunications = async () => {
    setLoading(true);
    try {
      const response = await laravelApiClient.get("/responsable/communications");
      setCommunications(response.data?.data || response.data || []);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await laravelApiClient.post("/responsable/communications", {
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingId(id);
    try {
      await laravelApiClient.delete(`/responsable/communications/${id}`);
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3 min-w-0 flex-1">
                    <div className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-500/10 flex items-center justify-center text-blue-600 dark:text-blue-400 shrink-0 mt-0.5">
                      <Megaphone size={18} />
                    </div>
